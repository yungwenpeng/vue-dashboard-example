...
export default defineComponent({
    ...
    created() {
        ...
        if (decoded['scopes'][0] === 'TENANT_ADMIN') {
            RestApiService.getTenantAllDevices(store.getters.getAuthUserToken['token'])
                .then(res => {
                    const data: any = res.data.data;
                    data.forEach((element: any) => {
                        //console.log('getDevices(TENANT) - element:', element);
                        this.devices.push({
                            id: element['id']['id'],
                            name: element['name'],
            ...
            RestApiService.getCustomerAllDevices(decoded['customerId'], store.getters.getAuthUserToken['token'])
                .then(res => {
                    const data: any = res.data.data;
                    data.forEach((element: any) => {
                        //console.log('getDevices(TENANT) - element:', element);
...
<template>
    ...
    <div class="offcanvas offcanvas-end text-bg-light" tabindex="-1" id="deviceDetail"
        aria-labelledby="offcanvasDarkNavbarLabel" style="min-width: 30%;">
        <div class="offcanvas-body">
            ...
            <div class="tab-content" id="nav-tabContent">
                <div class="tab-pane fade show active" id="nav-details" role="tabpanel"
                    ...
                    <div v-for="(item, index) in selectedDevice">
